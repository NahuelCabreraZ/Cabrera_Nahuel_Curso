...
personaDb.createPersona = function (persona, funCallback) {
    var query = 'INSERT INTO personas (dni,nombre,apellido,sexo,fecha_nacimiento) VALUES (?,?,?,?,?)'
    var dbParams = [personas.dni, personas.nombre, personas.apellido, personas.sexo, personas.fecha_nacimiento];
    connection.query(query, dbParams, function (err, result, fields) {
        if (err) {
            if(err.code == 'ER_DUP_ENTRY'){
                funCallback({
                    message: `Ya existe la persona con el DNI ${personas.dni}`,
                    detail: err
                });
            }else{
                funCallback({
            ...
            funCallback(undefined, {
                message: `Se creo la persona ${personas.apellido} ${personas.nombre}`,
                detail: result
            });
        }
    });
 ...
 * 
 */
personaDb.updatePersona = function (dni, persona, funCallback) {
    var query = 'UPDATE personas SET dni = ? , nombre = ?, apellido = ?,  sexo = ?, fecha_nacimiento = ?, estado = ? WHERE dni = ?'
    var dbParams = [personas.dni, personas.nombre, personas.apellido, personas.sexo, personas.fecha_nacimiento, personas.estado, dni];
    connection.query(query, dbParams, function (err, result, fields) {
        if (err) {
            funCallback({
                code:3,
            ...
            if (result.affectedRows == 0) {
                ...
                funCallback({
                    code:1,
                    message: `Se modifico la persona ${personas.apellido} ${personas.nombre}`,
                    detail: result
                });
            }
        }
...
personaDb.eliminarPersona = function(dni,funCallback){
 ...
 * 
 */
personaDb.eliminacionlogicaPersona = function (idpersonas, funCallback) {
    connection.query("UPDATE personas SET estado = 0 WHERE idpersona = ?",idpersonas, function (err, result, fields) {
        if (err) {
            funCallback({
                code:3,
                message: "Surgio un problema, contactese con un administrador. Gracias",
